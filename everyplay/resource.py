...
class Resource(object):
    ...
    Provides an object interface to resources returned by the Everyplay API.
    """
...
def wrapped_resource(response):
    ...
    for attr in ['url', 'status_code', 'ok']:
        setattr(result, attr, getattr(response, attr))

    error = None if response.ok else response.content
    setattr(result, 'error', error)
